...
import static org.assertj.core.api.Assertions.assertThat;
 ...
 class ApiKeyGeneratorTest {
    ...
    @DisplayName("two calls never produce the same key")
     void twoCallsDontProduceSameKey() {
        String key = apiKeyGenerator.newSecretKey();
    ...
    @DisplayName("hash() returns the same value for the same input and a different one for different input")
     void sameInputProducesSameHash() throws NoSuchAlgorithmException {
    ...
    @Test
    void hashDoesNotEqualPlainKey() throws NoSuchAlgorithmException {
       String key = apiKeyGenerator.newSecretKey();
       String hashedKey = apiKeyGenerator.hash(key);

       assertThat(key).isNotEqualTo(hashedKey);
    }
}
