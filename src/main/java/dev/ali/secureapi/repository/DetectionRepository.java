...
public class DetectionRepository {
    ...
    public List<RuleMatch> findIpBruteForce(OffsetDateTime windowStart, OffsetDateTime windowEnd, int threshold) {
        String sql = "SELECT host(source_ip) as entity, COUNT(*) as count FROM security_events WHERE event_type = 'AUTH_FAILURE' AND ((created_at >= :window_start) AND (created_at <= :window_end)) GROUP BY source_ip HAVING COUNT(*)  >= :threshold";
    ...
    public List<RuleMatch> findPasswordSpraying(OffsetDateTime windowStart, OffsetDateTime windowEnd, int threshold) {
        String sql = "SELECT host(source_ip) as entity, COUNT(*) as count FROM security_events WHERE event_type = 'AUTH_FAILURE' AND ((created_at >= :window_start) AND (created_at <= :window_end)) GROUP BY source_ip HAVING COUNT(DISTINCT principal) >= :threshold ";
        return jdbc.sql(sql).params(Map.of("window_start", Timestamp.from(windowStart.toInstant()), "window_end", Timestamp.from(windowEnd.toInstant()), "threshold", threshold)).query(RuleMatch.class).list();
    }
