                ...
                if (request.getRequestURI().startsWith("/api/alerts")) {
                    try {
                        ...
                        if (repoKey.isPresent()) {
                            ...
                            publisher.publishEvent(new SecurityContextEvent(this, SecurityEventType.API_KEY_REJECTED, null, Map.of("apiKeyHash", hash)));
                        }
                    ...
                    publisher.publishEvent(new SecurityContextEvent(this, SecurityEventType.AUTHZ_DENIED, null, Map.of("path", request.getRequestURI(), "method", request.getMethod())));
                    response.setStatus(HttpStatus.FORBIDDEN.value());
                    response.setContentType(MediaType.APPLICATION_JSON_VALUE);
                    response.setCharacterEncoding("UTF-8");
                    response.getWriter().print(objectMapper.writeValueAsString(ApiResponse.error("Invalid request", null)));
                    return;
                }
            }
